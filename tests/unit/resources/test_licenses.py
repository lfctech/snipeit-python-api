...
def test_list_licenses(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/licenses", json={"total": 1, "rows": [{"id": 1, "name": "Test License"}]})
...
def test_get_license(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/licenses/1", json={"id": 1, "name": "Test License"})
...
def test_create_license(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="POST", url="https://test.snipeitapp.com/api/v1/licenses", json={"status": "success", "payload": {"id": 2, "name": "New License"}})
...
def test_patch_license(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="PATCH", url="https://test.snipeitapp.com/api/v1/licenses/1", json={"status": "success", "payload": {"id": 1, "name": "Patched License"}})
...
def test_delete_license(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="DELETE", url="https://test.snipeitapp.com/api/v1/licenses/1", json={"status": "success", "messages": "License deleted"})
    snipeit_client.licenses.delete(1)
    assert len(httpx_mock.get_requests()) == 1

def test_save_license(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/licenses/1", json={"id": 1, "name": "Test License"})
    httpx_mock.add_response(method="PATCH", url="https://test.snipeitapp.com/api/v1/licenses/1", json={"status": "success", "payload": {"id": 1, "name": "Saved License"}})
    lic = snipeit_client.licenses.get(1)
