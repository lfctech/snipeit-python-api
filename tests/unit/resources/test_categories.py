...
def test_list_categories(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/categories", json={"total": 1, "rows": [{"id": 1, "name": "Test Category"}]})
...
def test_get_category(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/categories/1", json={"id": 1, "name": "Test Category"})
    category = snipeit_client.categories.get(1)
    assert isinstance(category, Category)
    assert category.name == "Test Category"

def test_create_category(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="POST", url="https://snipe.example.test/api/v1/categories", json={"status": "success", "payload": {"id": 2, "name": "New Category"}})
...
def test_patch_category(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/categories/1", json={"status": "success", "payload": {"id": 1, "name": "Patched Category"}})
...
def test_delete_category(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="DELETE", url="https://snipe.example.test/api/v1/categories/1", json={"status": "success", "messages": "Category deleted"})
    snipeit_client.categories.delete(1)
    assert len(httpx_mock.get_requests()) == 1

def test_save_category(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/categories/1", json={"id": 1, "name": "Test Category"})
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/categories/1", json={"status": "success", "payload": {"id": 1, "name": "Saved Category"}})
    category = snipeit_client.categories.get(1)
