...
def test_list_models(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/models", json={"total": 1, "rows": [{"id": 1, "name": "Test Model"}]})
...
def test_get_model(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/models/1", json={"id": 1, "name": "Test Model"})
    model = snipeit_client.models.get(1)
    assert isinstance(model, Model)
    assert model.name == "Test Model"

def test_create_model(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="POST", url="https://snipe.example.test/api/v1/models", json={"status": "success", "payload": {"id": 2, "name": "New Model"}})
...
def test_patch_model(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/models/1", json={"status": "success", "payload": {"id": 1, "name": "Patched Model"}})
...
def test_delete_model(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="DELETE", url="https://snipe.example.test/api/v1/models/1", json={"status": "success", "messages": "Model deleted"})
    snipeit_client.models.delete(1)
    assert len(httpx_mock.get_requests()) == 1

def test_save_model(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/models/1", json={"id": 1, "name": "Test Model"})
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/models/1", json={"status": "success", "payload": {"id": 1, "name": "Saved Model"}})
    model = snipeit_client.models.get(1)
