...
def test_list_fieldsets(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/fieldsets", json={"total": 1, "rows": [{"id": 1, "name": "Test Fieldset"}]})
...
def test_get_fieldset(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/fieldsets/1", json={"id": 1, "name": "Test Fieldset"})
...
def test_create_fieldset(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="POST", url="https://test.snipeitapp.com/api/v1/fieldsets", json={"status": "success", "payload": {"id": 2, "name": "New Fieldset"}})
...
def test_patch_fieldset(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="PATCH", url="https://test.snipeitapp.com/api/v1/fieldsets/1", json={"status": "success", "payload": {"id": 1, "name": "Patched Fieldset"}})
...
def test_delete_fieldset(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="DELETE", url="https://test.snipeitapp.com/api/v1/fieldsets/1", json={"status": "success", "messages": "Fieldset deleted"})
    snipeit_client.fieldsets.delete(1)
    assert len(httpx_mock.get_requests()) == 1

def test_save_fieldset(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/fieldsets/1", json={"id": 1, "name": "Test Fieldset"})
    httpx_mock.add_response(method="PATCH", url="https://test.snipeitapp.com/api/v1/fieldsets/1", json={"status": "success", "payload": {"id": 1, "name": "Saved Fieldset"}})
    fieldset = snipeit_client.fieldsets.get(1)
