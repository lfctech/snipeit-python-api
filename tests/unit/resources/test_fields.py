...
def test_create_field(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="POST", url="https://snipe.example.test/api/v1/fields", json={"status": "success", "payload": {"id": 2, "name": "New Field"}})
...
def test_patch_field(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/fields/1", json={"status": "success", "payload": {"id": 1, "name": "Patched Field"}})
...
def test_delete_field(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="DELETE", url="https://snipe.example.test/api/v1/fields/1", json={"status": "success", "messages": "Field deleted"})
    snipeit_client.fields.delete(1)
    assert len(httpx_mock.get_requests()) == 1

def test_save_field(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/fields/1", json={"id": 1, "name": "Test Field"})
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/fields/1", json={"status": "success", "payload": {"id": 1, "name": "Saved Field"}})
    field = snipeit_client.fields.get(1)
