...
def test_list_departments(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/departments", json={"total": 1, "rows": [{"id": 1, "name": "Test Department"}]})
...
def test_get_department(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/departments/1", json={"id": 1, "name": "Test Department"})
...
def test_create_department(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="POST", url="https://snipe.example.test/api/v1/departments", json={"status": "success", "payload": {"id": 2, "name": "New Department"}})
...
def test_patch_department(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/departments/1", json={"status": "success", "payload": {"id": 1, "name": "Patched Department"}})
...
def test_delete_department(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="DELETE", url="https://snipe.example.test/api/v1/departments/1", json={"status": "success", "messages": "Department deleted"})
    snipeit_client.departments.delete(1)
    assert len(httpx_mock.get_requests()) == 1

def test_save_department(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://snipe.example.test/api/v1/departments/1", json={"id": 1, "name": "Test Department"})
    httpx_mock.add_response(method="PATCH", url="https://snipe.example.test/api/v1/departments/1", json={"status": "success", "payload": {"id": 1, "name": "Saved Department"}})
    department = snipeit_client.departments.get(1)
