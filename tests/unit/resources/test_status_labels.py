...
def test_list_status_labels(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/statuslabels", json={"total": 1, "rows": [{"id": 1, "name": "Test StatusLabel"}]})
...
def test_get_status_label(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/statuslabels/1", json={"id": 1, "name": "Test StatusLabel"})
...
def test_create_status_label(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="POST", url="https://test.snipeitapp.com/api/v1/statuslabels", json={"status": "success", "payload": {"id": 2, "name": "New StatusLabel"}})
...
def test_patch_status_label(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="PATCH", url="https://test.snipeitapp.com/api/v1/statuslabels/1", json={"status": "success", "payload": {"id": 1, "name": "Patched StatusLabel"}})
...
def test_delete_status_label(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="DELETE", url="https://test.snipeitapp.com/api/v1/statuslabels/1", json={"status": "success", "messages": "StatusLabel deleted"})
    snipeit_client.status_labels.delete(1)
    assert len(httpx_mock.get_requests()) == 1

def test_save_status_label(snipeit_client, httpx_mock):
    httpx_mock.add_response(method="GET", url="https://test.snipeitapp.com/api/v1/statuslabels/1", json={"id": 1, "name": "Test StatusLabel"})
    httpx_mock.add_response(method="PATCH", url="https://test.snipeitapp.com/api/v1/statuslabels/1", json={"status": "success", "payload": {"id": 1, "name": "Saved StatusLabel"}})
    status_label = snipeit_client.status_labels.get(1)
